...
describe('точность шаблона на реальных данных', () => {
  it('шаблон воспроизводит каждый опубликованный день', () => {
    // Цикл построен по прошедшим неделям, а days — это будущие дни, которые
    // вуз уже опубликовал. Совпадают они только если расписание действительно
    // живёт двухнедельным циклом, так что проверка не пустая.
    //
    // На седьмом семестре (с 01.09.2026) цикл воспроизводит все двадцать
    // учебных дней без единого расхождения. Если этот тест упал — семестр
    // перестал быть регулярным, и пора пересобрать: `cli bake`, `npm run gen`.
    const shape = (list) =>
      list
        .map((l) => l.p + '|' + l.n)
        .sort()
    ...
    for (const day of baked.days) {
      ...
      if (!guess) {
        mismatched.push(day.d + ': нет слота ' + slot)
        continue
      }
      checked += 1
      if (shape(guess) !== shape(day.l)) {
        mismatched.push(day.d + ' (' + slot + ')')
      }
    }
