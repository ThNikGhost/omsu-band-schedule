 ...
 *
 * Точность измерена на реальных данных группы 5028. На текущем, седьмом
 * семестре цикл воспроизводит все двадцать учебных дней без единого
 * расхождения; на прошлых семестрах было 85–90%, потому что там среди
 * семестра начинались новые предметы. Регулярность — свойство конкретного
 * семестра, а не расписания вообще.
 *
 * Поэтому вычисленные дни всё равно помечаются `a: 1`, и экран показывает их
 * как приблизительные: сегодня семестр регулярен, но обещать это наперёд
 * нельзя, а обещать точность, которой может не быть, хуже, чем честно
 * сказать «примерно».
 *
